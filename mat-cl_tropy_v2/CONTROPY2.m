...
%         end
        ...
        [mat_A{ws}, nWindows{ws}] = overlapData(data_A,floor(length(data_A) / window_size),ovlr(ws));
        [mat_B{ws}, ~] = overlapData(data_B,floor(length(data_B) / window_size),  ovlr(ws));
...
%                 msMat_A{fi}{ws}(w,:) = (msMat_A{fi}{ws}(w,:) - mean(msMat_A{fi}{ws}(w,:))) ./ (std(msMat_A{fi}{ws}(w,:)).^2);
%                 msMat_B{fi}{ws}(w,:) = (msMat_B{fi}{ws}(w,:) - mean(msMat_B{fi}{ws}(w,:))) ./ (std(msMat_B{fi}{ws}(w,:)).^2);
                
                msMat_A{fi}{ws}(w,:) = (msMat_A{fi}{ws}(w,:) - min(msMat_A{fi}{ws}(w,:))) ./ (max(msMat_A{fi}{ws}(w,:)) - min(msMat_A{fi}{ws}(w,:)));
                msMat_B{fi}{ws}(w,:) = (msMat_B{fi}{ws}(w,:) - min(msMat_B{fi}{ws}(w,:))) ./ (max(msMat_B{fi}{ws}(w,:)) - min(msMat_B{fi}{ws}(w,:)));
            end
        end
...
%     fprintf ('pks & hst, ');
    ...
    for ws=1:length(wsizes)
        for sig=1:length(sigmas)
            ...
            [v,l] = max(cn);
            sacout{sig,ws} = [un{ws,sig}(l), v/length(cn)];
            cnc{ws,sig} = cn;
        end
    end
