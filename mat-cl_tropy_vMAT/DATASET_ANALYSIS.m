...
for d=1:length(datadir)
    ...
    [offset_mstyle,ranoff,off_pks] = W_CCC_Mstyle_CL(data, sigmas, wsize, overlap_ratio, ocl)
    offsetcac_bydir{dircount} = offset_mstyle;
    offsetran_bydir{dircount} = ranoff;
    offsetpks_bydir{dircount} = off_pks;
    ...
    for ws=1:nws
        ...
        for c=1:ncomb
            fprintf (afile, 'c. %d\t', c);
            for i=1:length(sigmas)
                fprintf (afile, '%.2f \t ', ranoff{c,i,ws});
            end
            fprintf (afile, '%.2f\n', proof_offset{dircount,c});
        end
    end

%     [offset_sil,sig_sil] = W_CCC_Mstyle_SILVERMAN_CL(data, wsize, overlap_ratio, ocl)
...
offset_ran_fullmat = zeros (length(offsetran_bydir), ncomb, nsigs, nws);
offset_pks_fullmat = zeros (length(offsetpks_bydir), ncomb, nsigs, nws);
...
for nd=1:length(offsetcac_bydir)
    for c=1:length(offsetcac_bydir{nd}(:,1,1))
        for ws=1:length(offsetcac_bydir{nd}(1,1,:))
            ...
            offset_ran_fullmat(nd,c,:,ws) = cell2mat(offsetran_bydir{nd}(c,:,ws));
            offset_pks_fullmat(nd,c,:,ws) = cell2mat(offsetpks_bydir{nd}(c,:,ws));
        end
    end
end

for nd=1:length(offsetcor_bydir)
    for c=1:length(offsetcor_bydir{nd}(:,1))
        for ws=1:length(offsetcor_bydir{nd}(c,:))
    ...
    for sig=1:nsigs
        for ws=1:nws
            ...
            for dir=1:length(offsetcac_bydir)
                ...
                if (offset_ran_fullmat(dir,c,sig,ws)>=proof_offset{dir,c}-2 && offset_ran_fullmat(dir,c,sig,ws)<=proof_offset{dir,c}+2)
                    binran = binran + 1;
                end
                if (offset_pks_fullmat(dir,c,sig,ws)>=proof_offset{dir,c}-2 && offset_pks_fullmat(dir,c,sig,ws)<=proof_offset{dir,c}+2)
                    binpks = binpks + 1;
                end
            ...
            medprobrate_ran(sig,ws,c) = binran / length(offsetran_bydir);
            medprobrate_pks(sig,ws,c) = binpks / length(offsetpks_bydir);
        end
    end
end
...
for c=1:ncomb
    subplot(2,3,c);
    mesh(wsize,sigmas,medprobrate_pks(:,:,c));
    title (['Probability rate CCC. PKS', num2str(c)]);
    xlabel ('window size');
    ylabel ('sigmas');
end
...
disp (['RANSAC CCC min|avg|max: ', num2str(ccc_min), ' | ', num2str(ccc_avg), ' | ', num2str(ccc_max)]);

ccc_max = max(max(max(medprobrate_pks)));
ccc_avg = mean(mean(mean(medprobrate_pks)));
ccc_min = min(min(min(medprobrate_pks)));
disp (['PKS CCC min|avg|max: ', num2str(ccc_min), ' | ', num2str(ccc_avg), ' | ', num2str(ccc_max)]);
