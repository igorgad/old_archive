...
for sig=1:nsigs
    for ws=1:nws
        ...
        medprobrate_ran(sig,ws) = binran / nan_comb;
        medprobrate_pks(sig,ws) = binpks / nan_comb;
    end
end
...
figure;
mesh(wsize,sigmas,medprobrate_ran(:,:));
title (['Probability rate CCC RANSAC']);
xlabel ('window size');
ylabel ('sigmas');

figure;
mesh(wsize,sigmas,medprobrate_pks(:,:));
